...
def build_tshark_argv(
    ...
    # occurrence=f: take only the FIRST value of a multi-valued field (e.g.
    # an ICMP error packet embeds the original packet's IP header, so
    # ip.src/ip.dst would otherwise appear twice per row). Without this,
    # tshark joins repeated values with the same "," used as the field
    # separator, producing a row with more commas than expected — this was
    # confirmed against the real Monday-WorkingHours capture, where it
    # silently corrupted ~0.25% of rows before being caught here.
    argv = [tshark_bin, "-r", str(pcap_path), "-T", "fields",
...
def _finalize_chunk(rows: list[list[str]], columns: list[str]) -> pd.DataFrame:
    ...
    # ip_src/ip_dst are never numeric-coerced (they're strings, not one of
    # _NUMERIC_COLS), so a missing value from tshark arrives as an empty
    # string, not NaN — dropna() alone would silently miss it. Normalize
    # empty strings to NaN first so both checks actually catch it.
    df["ip_src"] = df["ip_src"].replace("", pd.NA)
    df["ip_dst"] = df["ip_dst"].replace("", pd.NA)
...
def extract_pcap_to_parquet(
    ...
    try:
        for chunk in stream_tshark_chunks(pcap_path, time_filter=time_filter, chunk_size=chunk_size):
            ...
            n_chunks += 1
            if n_chunks % 10 == 0:
                logger.info("extract_pcap_to_parquet: %s -> %d rows written so far (%d chunks)",
                            pcap_path, total_rows, n_chunks)
    finally:
